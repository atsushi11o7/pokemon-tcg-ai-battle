...
class RunConfigTest(unittest.TestCase):
    def test_generalist_configs_select_exactly_one_algorithm(self) -> None:
        ppo = load_run_config(ROOT / "configs" / "ppo.yaml")
        mcts = load_run_config(ROOT / "configs" / "mcts.yaml")
        ...
        # 汎用の設定はどちらも重み無しから学習を始められること。
        self.assertIsNone(ppo.model.initial_checkpoint)
        self.assertIsNone(mcts.model.initial_checkpoint)
    ...
    def test_trainer_algorithm_mismatch_is_rejected(self) -> None:
        config = load_run_config(ROOT / "configs" / "ppo.yaml")

        with self.assertRaisesRegex(ValueError, "mcts"):
            validate_algorithm(config, "mcts")

    def _ppo_config(self) -> dict:
        with (ROOT / "configs" / "ppo.yaml").open(encoding="utf-8") as file:
