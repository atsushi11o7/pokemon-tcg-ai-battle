...
zipは展開せず、メンバーを1つずつ読んでシャードへ書き出す。

行動の列挙は`mcts.search.enumerate_actions`をそのまま使い、学習・推論と同じ関数を
通すことで教師信号のindexが食い違わないようにする。
"""
...
def extract_episode(
    ...
    *,
    ...
    (抽出CLIの既定はこちら)。`policy_target`を全ゼロにすると方策の勾配は出ず、
    価値の教師(`label`)だけが効く。
    ...
    Args:
        ...
        policy_seats: 指定するとその席にだけ方策の教師を付ける(勝敗を問わない)。
            勝ち負け両方から同じデッキの指し手だけを取り出すためのモード。
            ミラー戦では両席を渡せば2通り分取れる。
...
def drop_from_page_cache(path: Path) -> None:
    ...
    1エポックで全シャードを読み流すため、放っておくとページキャッシュが空きメモリを
    埋め尽くす。エポックごとに順序をシャッフルするので再利用もほとんど起きない。
    `/proc/sys/vm/drop_caches`はコンテナから書けないので、ファイル単位で指示する。
    失敗しても学習は続けられるので握り潰す。
    """
    try:
...
def shard_sample_counts(paths: list[Path], cache_dir: Path) -> list[int]:
    ...
    `torch.load`で全シャードを展開すると、数百シャードでは十数分かかる。
