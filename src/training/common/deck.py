"""デッキcsvの読み込みと、自己対戦モードごとの両サイドのデッキ決定。

モードの意味付けはMCTS(`mcts/selfplay.py`)とPPO(`ppo/selfplay.py`)で共通。
"""
...
def sample_deck(opponent_deck_pool: list[list[int]], role: str) -> list[int]:
    """WeightedDeckPoolなら役割別分布、通常のlistなら一様分布から選ぶ。"""
...
def pick_decks_and_collect_seats(
    ...
    Args:
        mode: "asymmetric"(固定デッキ対ランダムデッキ)、"mirror"(両者同デッキ)、
            "generalist"(両者とも実在デッキプールから独立ランダム)。いずれも両サイドを学習する。
