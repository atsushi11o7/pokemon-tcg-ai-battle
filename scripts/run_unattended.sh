...
uv run python -u scripts/meta_collect.py \
...
if ! uv run python -u scripts/meta_build_registry.py >> data/meta/registry.log 2>&1; then
...
uv run python -u -m training.cli validate --config configs/ppo.yaml || exit 1
uv run python -u -m training.cli train --config configs/ppo.yaml \
